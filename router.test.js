...
describe('OpenAPIRouter', () => {
    describe('.parseRequest', () => {
        const api = new router_1.OpenAPIRouter({ definition });
    ...
    describe('.matchOperation', () => {
        ...
        test('does not match GET /v2/pets', async () => {
            const operation = api.matchOperation({ path: '/v2/pets', method: 'get', headers });
            expect(operation).toBe(undefined);
        });
    });
    describe('.matchOperation with apiRoot = /api', () => {
        const api = new router_1.OpenAPIRouter({ definition, apiRoot: '/api' });
        ...
        test('does not match GET /pets', async () => {
            const operation = api.matchOperation({ path: '/pets', method: 'get', headers });
            expect(operation).toBe(undefined);
        });
    });
...
describe('OpenAPIBackend', () => {
    describe('.handleRequest', () => {
        const dummyHandlers = {};
        const dummyHandler = (operationId) => (dummyHandlers[operationId] = jest.fn(() => ({ operationId })));
        const api = new backend_1.OpenAPIBackend({
            definition,
            handlers: {
    ...
    describe('.handleRequest postResponseHandler', () => {
        ...
        test('handles GET / and passes response to postResponseHandler', async () => {
            const postResponseHandler = jest.fn((c) => c && c.response);
